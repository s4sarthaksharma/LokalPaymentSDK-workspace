 ...
 * property initialize before `main()`, which touches [JuspayGatewayHandler]
 * and runs its registering `init` block — so the gateway is registered with
 * zero host code, same as Android (mirrors RazorpayCheckoutEagerInit.kt).
 * Registration alone doesn't make Juspay payable yet — the host's
 * [JuspaySdk.configure] call is still required.
 *
 ...
 *   [JuspayGatewayHandler]'s `init` block) a bare in-memory registration — no
 *   logging, no UIKit, no config reads.
 */
@Suppress("DEPRECATION", "unused")
@EagerInitialization
private val eagerRegistration = JuspayGatewayHandler
