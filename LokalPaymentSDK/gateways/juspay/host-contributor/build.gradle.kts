...
// :razorpay-checkout's contributor, kept in its own package (…juspay.host) so the two
...
// OWNED_MODULE constant, so JuspayHostContributor.module can't drift from the artifactId
...
// ServiceLoader (see JuspayHostContributor). Published at its module coordinate so the
