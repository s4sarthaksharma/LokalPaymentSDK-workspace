...
:razorpay-upi-intent — new module, Android-only, modeled on matrimony-kmp's UpiIntent client
```
...
## `:razorpay-upi-intent` — new module, Android-only
...
`AndroidRazorpayPaymentClient.kt` (UpiIntent branch) and `RazorpayUtil.kt`:

- commonMain:
  - `RazorpayUpiIntentConfig.kt` — `razorpayKey: String`, `data: JsonObject`
    (parsed from `CreateOrderResponse.gatewayConfig` the same lenient way
    `CreateOrderResponseMapper.kt:17-23` does today, gated on
    `PaymentGateway.RAZORPAY_INTENT` instead of `RAZORPAY_CHECKOUT` — that
    enum value already exists at `model/PaymentGateway.kt:12`, reserved and
    unused until now)
  - `RazorpayUpiIntentResultListener.kt`, `RazorpayUpiIntentResultMapper.kt`
    ...
    not `0` like Checkout.
- androidMain:
  - `AndroidRazorpayUpiIntentClient.kt` — wraps
  ...
  - `RazorpayUpiIntentSdk.kt` — the `pay()` facade:
...
2. Host's Activity must forward `onActivityResult` to the client (some UPI
   ...
   Expose this as `AndroidRazorpayUpiIntentClient.onActivityResult(...)`,
...
- Each new module's `build.gradle.kts`: `group = "com.getlokalapp.paymentsdk"`,
  ...
  `com.getlokalapp.paymentsdk.checkout` / `...upiintent`).
...
- `LokalPaymentSDKDemo/gradle/libs.versions.toml:21` declares
  ...
  (adding `:razorpay-upi-intent:publishToMavenLocal` once the demo exercises it).
- `composeApp/build.gradle.kts:53` (`implementation(libs.lokalpaymentsdk.shared)`)
  ...
  exercises hosted Checkout today, so `razorpay-upi-intent` isn't wired into
...
1. `./gradlew :shared:build :razorpay-checkout:build :razorpay-upi-intent:build`
