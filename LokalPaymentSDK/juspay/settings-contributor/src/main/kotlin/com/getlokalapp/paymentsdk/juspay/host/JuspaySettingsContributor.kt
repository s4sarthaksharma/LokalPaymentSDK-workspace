 ...
 * and pins a default version for the sibling `juspay-host` project plugin. Discovered
 * by the umbrella `com.getlokalapp.paymentsdk.lokal-payment-settings` plugin via
 * ServiceLoader.
 *
 ...
 * nothing Juspay happens unless the host applies `juspay-host` / imports `:juspay`.
 */
class JuspaySettingsContributor : LokalGatewaySettingsContributor {
    ...
    override fun contribute(settings: Settings) {
        ...
        // Default version for the sibling project plugin, so the host's app module can
        // write `plugins { id("com.getlokalapp.paymentsdk.juspay-host") }` (or a
        // versionless catalog alias) with no version anywhere — the umbrella settings
        // plugin's own version (pinned once in the host's settings.gradle.kts) is the
        // only Juspay-related version a host ever declares.
        settings.pluginManagement.plugins.id(HOST_PLUGIN_ID).version(hostPluginVersion())
    }

    // Both the umbrella and the juspay-host plugin publish from the same build at the
    ...
    private companion object {
        ...
        // transitive dependency of JuspayHostPlugin, via pluginManagement) and for the
